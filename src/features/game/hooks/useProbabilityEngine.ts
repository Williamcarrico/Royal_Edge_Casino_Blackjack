...
import { BlackjackProbabilityEngine, DeckComposition, PlayerDecisionProbabilities, DealerOutcomeProbabilities, HouseEdgeInfo } from './ProbabilityEngine';
...
export const useProbabilityEngine = (): ProbabilityEngineState => {
    ...
    const convertToEngineRules = (settings: any): StoreGameRules => {
        return {
    ...
    const probabilityEngine = useMemo(() => {
        ...
        settingsRules?.decksCount,
        settingsRules?.dealerHitsSoft17,
        settingsRules?.blackjackPayout,
        settingsRules?.surrender,
        settingsRules?.doubleAfterSplit
    ]);
