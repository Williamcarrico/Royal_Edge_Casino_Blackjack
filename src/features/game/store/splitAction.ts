...
export function splitAction(
    ...
    const hand1 = handUtils.createSplitHand(card1, `split-1-${Date.now()}`);
    const hand2 = handUtils.createSplitHand(card2, `split-2-${Date.now()}`);
    ...
    // Add cards to hands
    const updatedHand1 = handUtils.addCard(hand1, newCard1);
    const updatedHand2 = handUtils.addCard(hand2, newCard2);
