...
function processDealerTurn(
    ...
    // Make sure all dealer cards are face up for display
    const allVisibleDealerHand = {
        ...finalDealerHand,
        cards: finalDealerHand.cards.map(card => ({
            ...card,
            isFaceUp: true
        }))
    };
    ...
    determineGameResults(state, set, allVisibleDealerHand, currentShoe, runningCount, trueCount, dealtCards);
}
...
function playDealerHandToCompletion(
    ...
    while (shouldDealerHit(currentHand) && shoe.length > 0 && hitCount < MAX_HITS) {
        ...
        // Ensure card is face up - Enforce this for all dealer cards
        const faceUpCard = {
            ...card,
            isFaceUp: true // Explicit setting for clarity
        };
    ...
    // Ensure all cards in the final hand are face up
    const finalHand = {
        ...currentHand,
        cards: currentHand.cards.map(card => ({
            ...card,
            isFaceUp: true
        }))
    };
    ...
    console.log('[playDealerHandToCompletion] Final dealer hand:',
        finalHand.cards.map(c => `${c.rank}${c.suit[0]}`),
        'Final values:', finalHand.values,
        'Best value:', handUtils.getBestValue(finalHand),
        'Busted:', finalHand.isBusted);
