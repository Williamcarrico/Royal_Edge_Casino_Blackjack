...
const addHandToEntities = (state: StoreGameState, hand: Hand): StoreGameState => {
    ...
    hand.cards.forEach(card => {
        if (state.entities && !state.entities.cards[card.id]) {
...
// Export utility functions to resolve "unused vars" warnings
export const updateHandInEntities = (state: StoreGameState, hand: Hand): StoreGameState => {
    if (!state.entities) {
...
export const removeHandFromEntities = (state: StoreGameState, handId: string): StoreGameState => {
    if (!state.entities?.hands?.[handId]) {
        return state;
    }

    // Remove the hand
    if (state.entities?.hands) {
    ...
    if (state.playerHandIds?.includes(handId)) {
        state.playerHandIds = state.playerHandIds.filter((id: string) => id !== handId);
    }

    return state;
};

// Extend the GameActions interface to include forceBettingPhase
interface ExtendedGameActions extends GameActions {
    forceBettingPhase: () => void;
}

/**
 * Create the game store using Zustand with the immer middleware
 */
export const useGameStore = create<StoreGameState & ExtendedGameActions>()(
    immer((set, get) => {
        ...
        return {
            ...
            canDoubleDown: () => {
                ...
                // Get current hand info for strategic evaluation
                ...
                // Traditional rule: only double on first two cards
                const isFirstTwoCards = playerHand.cards.length === 2;

                // Additional strategic scenarios where doubling makes sense
                const isStrategicDoubleScenario =
                    (handValue >= 9 && handValue <= 11) &&
                    (dealerUpCardValue >= 2 && dealerUpCardValue <= 6);

                // Allow doubling if either condition is met
                return (isFirstTwoCards || isStrategicDoubleScenario) && state.isDoubleDownAvailable;
            },
...
export const selectPlayerHands = (state: StoreGameState) => {
    ...
    if (state.playerHandIds?.length && state.entities?.hands) {
        return state.playerHandIds.map((id: string) => state.entities?.hands?.[id]).filter(Boolean);
    }
...
function processSplitHandsResults(
    ...
    // Define an interface for Hand with optional bet property
    interface HandWithBet extends Hand {
        bet?: number;
    }
    ...
    results.forEach((result, index) => {
        ...
        const handBet = (hand as HandWithBet).bet ?? defaultBet;

        // Use proper type handling for result
        ...
        totalWinAmount += processSingleHandResult(hand, handBet, resultString, stats);
    });
